...
class BradescoBoletoAdapter:
    def gerar_pdf(self, titulo, cedente, sacado, banco_cfg, caminho):
        os.makedirs(os.path.dirname(caminho), exist_ok=True)

        # Fallback: usa layout genérico via BoletoItau, mas aplica logo do banco
        try:
        ...
        logo_path = banco_cfg.get("logo_path") or os.path.join(base_dir, "Logos", variation, str(banco_cfg.get("codigo_banco", "237")) + ".bmp")
        try:
        ...
        if b:
            ...
            # Layout mínimo em caso de ausência de lib
