...
def importar(request, slug):
    ...
    try:
        ...
        if request.method == "POST" and form.is_valid():
            ...
            messages.success(
                ...
                "Conciliação {} importada: {} transações.".format(
                    resultado["numero_conciliacao"], resultado["total_importado"]
                ),
            )
            return redirect(
                "conciliacao:detalhe", slug=ctx.slug,
        ...
        messages.error(request, "Não foi possível acessar os dados da conciliação. Tente novamente mais tarde.")
        status = 503
    return render(request, "conciliacao/importar.html", {
