...
from .models import Osflorestal
...
class GerarTitulosOS(APIView):
    def post(self, request, slug=None):
        ...
        data = request.data
        osfl_orde = data.get("osfl_orde")
        osfl_forn = data.get("osfl_forn")
        osfl_tota = Decimal(data.get("osfl_tota", 0))
        ...
        if not osfl_orde or not osfl_forn or not osfl_tota:
            return Response({"detail": "osfl_orde, osfl_forn e osfl_tota são obrigatórios."}, status=400)

        # Buscar a ordem de serviço para pegar empresa e filial
        try:
            ordem = Osflorestal.objects.using(banco).get(osfl_orde=osfl_orde)
            osfl_empr = ordem.osfl_empr
            osfl_fili = ordem.osfl_fili
        except Osflorestal.DoesNotExist:
            return Response({"detail": "Ordem de serviço não encontrada."}, status=404)

        valor_parcela = (osfl_tota / parcelas).quantize(Decimal("0.01"))
        
        # Ajuste para garantir que a soma das parcelas seja igual ao total
        diferenca = osfl_tota - (valor_parcela * parcelas)
        ...
        for i in range(parcelas):
            ...
            titulos.append(Titulosreceber(
                ...
                titu_hist=f"Título gerado da OS {osfl_orde}",
            ))
...
class RemoverTitulosOSView(APIView):
    def post(self, request, slug=None):
        ...
        osfl_orde = request.data.get("osfl_orde")

        if not osfl_orde:
            return Response({"detail": "osfl_orde é obrigatório."}, status=400)
        ...
        titulos = Titulosreceber.objects.using(banco).filter(
            titu_empr=ordem.osfl_empr,
            titu_fili=ordem.osfl_fili,
            titu_seri="ORS",
            titu_titu=str(osfl_orde)
        )
    ...
    def get(self, request, orde_nume, slug=None):
        ...
        try:
            ...
            ordem = Osflorestal.objects.using(banco).get(
                osfl_empr=request.query_params.get('empr'),
                osfl_fili=request.query_params.get('fili'),
                osfl_orde=orde_nume
            )
        except Osflorestal.DoesNotExist:
            return Response({"detail": "Ordem de serviço não encontrada."}, status=404)
