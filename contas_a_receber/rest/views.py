...
class TitulosreceberViewSet(ModuloRequeridoMixin,viewsets.ModelViewSet):
    ...
    def historico_baixas(self, request, *args, **kwargs):
        ...
        baixas = Baretitulos.objects.using(banco).only(
            ...
            (models.Q(bare_dpag__isnull=True) | models.Q(bare_dpag__gte=date(1900,1,1))),
            (models.Q(bare_emis__isnull=True) | models.Q(bare_emis__gte=date(1900,1,1))),
            (models.Q(bare_venc__isnull=True) | models.Q(bare_venc__gte=date(1900,1,1))),
        ).order_by('-bare_dpag')
