...
def install_stubs_bradesco():
    ...
    ret = types.ModuleType('cnab240.retornos.bradesco')
    ...
    class _Ret400:
        def __init__(self, caminho):
            self.titulos = [types.SimpleNamespace(nosso_numero='1234567890', valor_pago=100.0, data_pagamento=__import__('datetime').date(2024,2,1))]

    class _TituloRet:
        def __init__(self):
            self.nosso_numero = '1234567890'
            self.valor_pago = 100.0
            self.data_pagamento = __import__('datetime').date(2024, 2, 1)

    class BradescoRetorno:
        def __init__(self, caminho):
            self.titulos = [_TituloRet()]

    ret.BradescoRetorno = BradescoRetorno
    ...
    # remessa.bradesco já configurada em install_all_cnab_stubs
    sys.modules['cnab240.retornos.bradesco'] = ret
...
class TestFluxoPagamentoBradesco(unittest.TestCase):
    def setUp(self):
        install_stubs_bradesco()
        ...
        self.banco_cfg = {"codigo_banco": "237", "agencia": "1234", "conta": "56789", "dv": "0", "carteira": "09", "logo_variation": "Colorido"}
        self.outdir = os.path.join("media", "tests", "bancos", "bradesco")
        os.makedirs(self.outdir, exist_ok=True)

    def test_pdf_remessa_retorno(self):
        pdf_path = os.path.join(self.outdir, "boleto_bradesco.pdf")
