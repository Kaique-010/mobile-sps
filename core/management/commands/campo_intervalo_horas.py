...
def campoIntervaloHoras(alias: str):
    with connections[alias].cursor() as cursor:
        cursor.execute(
            """
        alter table os_hora
        add column if not exists os_hora_manh_inte boolean default false;
...
class Command(BaseCommand):
    help = "Atualiza apenas o campo os_hora_manh_inte em todos os bancos de licenças"
    ...
    def handle(self, *args, **options):
        ...
        for lic in licencas:
            ...
            self.stdout.write(self.style.WARNING(f"[{alias}] Atualizando campo os_hora_manh_inte..."))
